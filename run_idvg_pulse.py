...
class AutoIdVgPulseWorker(BaseMeasurementWorker):
    ...
    def run(self):
        ...
        try:
            ...
            for config_idx, config_file in enumerate(self.config_files):
                ...
                try:
                    filename = self._setup_files(params, prefix="idvg_pulse")
                ...
                self._execute_idvg_pulse_measurement(filename, params, config_idx)
    ...
    def _execute_idvg_pulse_measurement(self, filename, params, config_idx):
            ...
            for vg in vg_points:
                ...
                if reading and len(reading) == 2:
                    ...
                    if I_D is not None:
                        ...
                        writer.writerow([vd_const, vg, I_D_clampped, I_G_clampped])
                        packet = SweepData(Vd=vd_const, Vg=vg, Id=I_D_clampped, Ig=I_G_clampped)
...
class AutoIdVgPulseWindow(BaseMeasurementWindow):
    def __init__(self, worker):
        super().__init__(worker, window_title="Pulsed Id-Vg")
...
if __name__ == "__main__":
    ...
    worker = AutoIdVgPulseWorker(RESOURCE_ID, config_queue, laser=laser)
    window = AutoIdVgPulseWindow(worker)
    window.show()
    sys.exit(app.exec_())