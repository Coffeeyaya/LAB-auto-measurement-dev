...
class AutoIdVdPulseWorker(BaseMeasurementWorker):
    ...
    def run(self):
        ...
        try:
            ...
            for config_idx, config_file in enumerate(self.config_files):
                ...
                try:
                    filename = self._setup_files(params, prefix="idvd_pulse")
                ...
                self._execute_idvd_pulse_measurement(filename, params, config_idx)
    ...
    def _execute_idvd_pulse_measurement(self, filename, params, config_idx):
        ...
        with open(filename, 'w', newline='') as f_csv:
            ...
            for vd in vd_points:
                ...
                # self.k.set_Vd(vd)
                # time.sleep(delay)
                reading = self.k.measure_pulsed_vd_vg(target_vd=vd, target_vg=vg_const, 
                                                      base_vd=base_vd, base_vg=base_vg, 
...
class AutoIdVdPulseWindow(BaseMeasurementWindow):
    def __init__(self, worker):
        super().__init__(worker, window_title="Pulsed Id-Vd", initial_log=True)
    ...
    def toggle_scale(self):
        ...
        for step_idx, mem in self.data_memory.items():
            ...
            if self.is_log:
                ids_plot = [max(1e-13, abs(x)) for x in ids_plot]
            self.lines_dict[step_idx].set_data(mem["vds"], ids_plot)
...
if __name__ == "__main__":
    ...
    worker = AutoIdVdPulseWorker(RESOURCE_ID, config_queue, laser=laser)
    window = AutoIdVdPulseWindow(worker)
    window.show()
    sys.exit(app.exec_())